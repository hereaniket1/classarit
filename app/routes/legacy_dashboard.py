...
def dashboard(
    ...
    sessions = (
        ...
        .join(models.Student)
        .filter(models.Student.teacher_id == teacher.id)
        .options(selectinload(models.ClassSession.student))
    ...
    pending_payments = sum(
        max((p.amount_due or 0) - (p.amount_paid or 0), 0)
        for p in payments
        if p.status in {"Due", "Partial", "Overdue"}
    )
    monthly_revenue = sum(
        p.amount_paid or 0
        for p in payments
    ...
    materials = (
        ...
        .order_by(models.Material.created_date.desc())
        .all()
    )
    students = (
        db.query(models.Student)
